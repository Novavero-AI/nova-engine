...
NvSkinPipeline *nv_skin_pipeline_create(NvDevice *dev,
    ...
    if (!dev || !vert_path || !frag_path || render_pass == VK_NULL_HANDLE) {
        return NULL;
    }

    NvSkinPipeline *pip = calloc(1, sizeof(NvSkinPipeline));
    if (!pip) return NULL;
    pip->device = dev->handle;

    /* ---- Bone SSBO descriptor set layout ---- */
    VkDescriptorSetLayoutBinding bone_binding;
    memset(&bone_binding, 0, sizeof(bone_binding));
    bone_binding.binding         = 0;
    bone_binding.descriptorType  = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    bone_binding.descriptorCount = 1;
    bone_binding.stageFlags      = VK_SHADER_STAGE_VERTEX_BIT;

    VkDescriptorSetLayoutCreateInfo bone_dsl;
    memset(&bone_dsl, 0, sizeof(bone_dsl));
    bone_dsl.sType        = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    bone_dsl.bindingCount = 1;
    bone_dsl.pBindings    = &bone_binding;

    if (vkCreateDescriptorSetLayout(pip->device, &bone_dsl, NULL,
                                     &pip->bone_set_layout) != VK_SUCCESS) {
        goto fail;
    }

    /* ---- Pipeline layout: existing sets + bone set ---- */
    uint32_t total_sets = set_layout_count + 1;
    ...
    all_layouts[set_layout_count] = pip->bone_set_layout;
    ...
    push.offset     = 0;
    push.size       = NV_PUSH_CONSTANT_SIZE; /* 128 bytes */
    ...
    VkResult layout_result = vkCreatePipelineLayout(
        pip->device, &pli, NULL, &pip->layout);
    free(all_layouts);
    ...
    attrs[3].offset   = 32;
    /* bone indices V4 (as floats, cast to ivec4 in shader) */
    attrs[4].location = 4;
    ...
    /* ---- Standard pipeline state (matches PBR pipeline) ---- */
    ...
    VkResult result = vkCreateGraphicsPipelines(
        pip->device, VK_NULL_HANDLE, 1, &ci, NULL, &pip->pipeline);
...
fail:
    nv_skin_pipeline_destroy(pip);
    return NULL;
}

void nv_skin_pipeline_destroy(NvSkinPipeline *pip) {
    if (!pip) return;
    if (pip->pipeline != VK_NULL_HANDLE)
        vkDestroyPipeline(pip->device, pip->pipeline, NULL);
    if (pip->layout != VK_NULL_HANDLE)
        vkDestroyPipelineLayout(pip->device, pip->layout, NULL);
    if (pip->bone_set_layout != VK_NULL_HANDLE)
        vkDestroyDescriptorSetLayout(pip->device,
                                      pip->bone_set_layout, NULL);
    free(pip);
}

VkDescriptorSetLayout nv_skin_pipeline_bone_layout(
    const NvSkinPipeline *pip) {
    return pip ? pip->bone_set_layout : VK_NULL_HANDLE;
}
