...
foreign import ccall unsafe "nv_swapchain_create"
  c_nv_swapchain_create :: Ptr () -> Ptr () -> Ptr () -> IO (Ptr ())
...
createSwapchain ::
  Instance -> Device -> Window -> IO (Maybe Swapchain)
createSwapchain inst dev win =
  withInstancePtr inst $ \instPtr ->
    withDevicePtr dev $ \devPtr ->
      withWindowPtr win $ \winPtr -> do
        ptr <- c_nv_swapchain_create instPtr devPtr winPtr
        if ptr == nullPtr
          then pure Nothing
          else do
